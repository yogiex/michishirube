...
export const ERROR_CATALOG: Record<ErrorCodeValue, ErrorCatalogEntry> = {
  ...
  [ErrorCode.RBAC_RESOURCE_DENIED]: {
    status: 403,
  ...
  [ErrorCode.VALIDATION_PARAM_INVALID]: {
    status: 400,
  ...
  [ErrorCode.VALIDATION_CONTENT_TYPE]: {
    status: 415,
  ...
  [ErrorCode.ROUTE_METHOD_NOT_ALLOWED]: {
    status: 405,
  ...
  [ErrorCode.ROUTE_UPSTREAM_MISSING]: {
    status: 500,
    title: 'Upstream not configured',
    retryable: false,
    hideDetailInProd: true,
  },
  [ErrorCode.ROUTE_CONFIG_INVALID]: {
    status: 500,
    title: 'Invalid route configuration',
    retryable: false,
    hideDetailInProd: true,
  },
  [ErrorCode.ROUTE_LOOP_DETECTED]: {
    status: 508,
  ...
  [ErrorCode.RATE_LIMIT_TENANT_EXCEEDED]: {
    status: 429,
    title: 'Tenant rate limit exceeded',
    retryable: true,
  },
  [ErrorCode.RATE_LIMIT_ROUTE_EXCEEDED]: {
    status: 429,
    title: 'Route rate limit exceeded',
    retryable: true,
  },
  [ErrorCode.RATE_LIMIT_BURST_EXCEEDED]: {
    status: 429,
  ...
  [ErrorCode.QUOTA_MONTHLY_EXCEEDED]: {
    status: 429,
  ...
  // Idempotency
  [ErrorCode.IDEMP_KEY_MISSING]: {
    status: 400,
    title: 'Idempotency key missing',
    retryable: false,
  },
  [ErrorCode.IDEMP_KEY_INVALID]: {
    status: 400,
  ...
  [ErrorCode.IDEMP_STORE_UNAVAILABLE]: {
    status: 503,
  ...
  [ErrorCode.CIRCUIT_HALF_OPEN_REJECTED]: {
    status: 503,
  ...
  [ErrorCode.UPSTREAM_RETRY_EXHAUSTED]: {
    status: 504,
    title: 'Upstream retries exhausted',
    retryable: false,
  },
  ...
  // Cache
  [ErrorCode.CACHE_STORE_UNAVAILABLE]: {
    status: 503,
    title: 'Cache store unavailable',
    retryable: true,
  },
  [ErrorCode.CACHE_KEY_INVALID]: {
    status: 500,
  ...
  [ErrorCode.INTERNAL_DEPENDENCY_ERROR]: {
    status: 500,
  ...
  [ErrorCode.OPS_METRICS_UNAVAILABLE]: {
    status: 503,
